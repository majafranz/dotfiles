...
globals.map('i', '<RIGHT>', '<Plug>(doge-comment-jump-forward)', silenced)
globals.map('i', '<LEFT>', '<Plug>(doge-comment-jump-backward)', silenced)
globals.map('x', '<RIGHT>', '<Plug>(doge-comment-jump-forward)', silenced)
globals.map('x', '<LEFT>', '<Plug>(doge-comment-jump-backward)', silenced)

-- lsp
globals.map('n', 'gD', '<cmd>lua vim.lsp.buf.declaration()<CR>', silenced)
globals.map('n', 'gd', '<cmd>lua vim.lsp.buf.definition()<CR>', silenced)
globals.map('n', 'gi', '<cmd>lua vim.lsp.buf.implementation()<CR>', silenced)
globals.map('n', 'gr', '<cmd>lua vim.lsp.buf.references()<CR>', silenced)
globals.map('n', '<leader>e', '<cmd>lua vim.diagnostic.open_float()<CR>', silenced)
globals.map('n', ']d', '<cmd>lua vim.diagnostic.goto_next()<CR>', silenced)
globals.map('n', '[d', '<cmd>lua vim.diagnostic.goto_prev()<CR>', silenced)
globals.map('n', 'K', '<cmd>lua vim.lsp.buf.hover()<CR>', silenced)
globals.map('n', '<C-k>', '<cmd>lua vim.lsp.buf.signature_help()<CR>', silenced)
globals.map('n', '<leader>rn', '<cmd>lua vim.lsp.buf.rename()<CR>', silenced)
globals.map('n', '<leader>ca', '<cmd>lua vim.lsp.buf.code_action()<CR>', silenced)
globals.map('n', '<leader>f', '<cmd>lua vim.lsp.buf.format({timeout_ms = 2000})<CR>', silenced)
